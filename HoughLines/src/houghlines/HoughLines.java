...
public class HoughLines {
    ...
    public static void main(String[] args) {
        ...
        String fileName = args.length >= 1 ? args[0] : "eye1.jpg"; // if no params provided, compute the defaut image
         ...
         */
            ...
            lines = cvHoughLines2(dst, storage, CV_HOUGH_STANDARD, 1, Math.PI / 4, 3, 1, 0);
            for (int i = 0; i < lines.total(); i++) {
                CvPoint2D32f point = new CvPoint2D32f(cvGetSeqElem(lines, i));
                ...
                double a = Math.cos((double) theta), b = Math.sin((double) theta);
                double x0 = a * rho, y0 = b * rho;
                CvPoint pt1 = new CvPoint((int) Math.round(x0 + 100 * (-b)), (int) Math.round(y0 + 100 * (a))), pt2 = new CvPoint((int) Math.round(x0 - 100 * (-b)), (int) Math.round(y0 - 100 * (a)));
                System.out.println("Line spotted: ");
                System.out.println("\t rho= " + rho);
                System.out.println("\t theta= " + theta);
                cvLine(colorDst, pt1, pt2, CV_RGB(255, 0, 0), 3, CV_AA, 0);
